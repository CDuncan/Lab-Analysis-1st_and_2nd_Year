...
%% Value Calculation 
    %% Right Principal Plane
    RPP_Row       = RightPrincipal;                     eRPP_Row      = ePrincipal;
    
    %% Image Distance 
    SPos_Row   = WMeanRow(SPos,eSPos);      eSPos_Row   = eWMeanRow(eSPos);
    V_Row          = SPos_Row - RPP_Row;       eV_Row   = Quadrature(eSPos_Row,eRPP_Row);    

    %% Image Size
    Size_Image   = Data_ScreenFocus(3).Reading;         eSize_Image   = InpError(Size_Image,2,'abs');
