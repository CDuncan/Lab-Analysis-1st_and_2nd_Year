...
[LPos,eLPos] = ConvexCorrection([Data_ScreenFocus(1).Reading]); %Postions relative to light box
[SPos,eSPos] = ScreenCorrection([Data_ScreenFocus(2).Reading]);

%% Value Calculation 
D_Row       = WMeanRow(SPos,eSPos);     eD_Row      = eWMeanRow(eSPos);
V_Row       = D_Row - LPos;             eV_Row      = Quadrature(eD_Row,eLPos);    

    %% Data Reuse
    U   = [LPos;V_Row];             eU  = [eLPos;eV_Row];
...
%disp([F3,eF3;NaN,ReducedChisq;RChiSqBounds])
